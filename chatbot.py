...
class ChatBotWindow(GridLayout):
    ...
    def send_message(self, _):
        ...
        if message:
            ...
            self.history.update_chat_history('[color=dd2020]Usuario[/color] >' + str(message))

            bot_response = self.chatbot.get_response(message)
            vali = Validar()

            vali.validar(bot_response)


            self.history.update_chat_history('[color=20dd20]ChatBot[/color] > ' + str(bot_response))
    ...
    def incoming_message(self, username, message):
        ...
        self.history.update_chat_history('[color=20dd20]Usuario[/color] > ' + str(message))
...
class ScrollableLabel(ScrollView):
    ...
    def update_chat_history(self, message):
        ...
        texto = Label(markup=True, text=message, size_hint=(Window.size[0], None))
        #texto.size = texto.texture_size
        #texto.width: txto.texture_size[0] + dp(10)

        #self.layout.height = self.minimum_height
        #texto.height = texto.texture_size[1]
        #texto.text_size = (texto.width * 0.98, None)
